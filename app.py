...
def chart_heatmap(sens_df,cur_price):
    ...
    for _,row in sens_df.iterrows():
        zr,tr=[],[]
        for v in row.values:
            try: val=float(v); zr.append(val); tr.append(f"${val:.1f}")
            except Exception: zr.append(None); tr.append("—")
        z.append(zr); text.append(tr)
    fig=go.Figure(go.Heatmap(z=z,x=list(sens_df.columns),y=list(sens_df.index),
